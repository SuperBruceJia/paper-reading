...
想要包含的非头文件应谨慎使用，并以`.inc`结尾。

<details>
<summary>详细信息</summary>

- 所有头文件要能够自给自足，即用户和重构工具不需要为特别场合而包含额外的头文件和符号（symbols）。详言之，一个头文件要有[头文件保护](#header.guard)， 并且包含它所需要的所有头文件和符号。

- 如果 `.h` 文件声明了一个模板和内联函数，应同时在该文件中加以定义。凡是有用到这些的 `.cc` 文件，就必须包含对应的头文件，否则程序可能会在构建中链接失败。如果声明和定义不在同一个文件中，声明应可传递的包含在定义中。禁止将定义分离到 `-inl.h` 文件中；过去这种做法很普遍，但现在不再允许。

- 有个例外：如果某函数模板为所有相关模板参数显式实例化，或本身就是某类的一个私有成员，那么它就只能定义在实例化该模板的 `.cc` 文件里。

- 在极少数情况下，有些头文件会被设计成不独立的。他们通常包含在被包含在不寻常的位置，例如作为文本插入到代码某处。他们可能没有头文件保护，也可能不包括其他先决条件。这些文件就要用 `.inc` 结尾，并谨慎使用，尽可能使用自给自足的头文件。
